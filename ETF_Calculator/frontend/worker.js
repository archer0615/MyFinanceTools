self.onmessage = (event) => {
  ...
  const result = runMonteCarlo(input, iterations, seed || 42);
  self.postMessage({ type: "complete", result });
};

function runMonteCarlo(input, iterations, seed) {
  const random = createSeededRandom(seed);
  const values = Array.from({ length: iterations }, () => runPath(input, random)).sort((a, b) => a - b);
