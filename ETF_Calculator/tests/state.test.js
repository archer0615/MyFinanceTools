...
test("state manager migrates legacy config into canonical state", () => {
  ...
  assert.equal(state.portfolio.baseCurrency, "TWD");
...
test("setError stores worker failure in ui state", () => {
  ...
  assert.equal(manager.getState().ui.isLoading, false);
});

test("loading state toggles through service layer", () => {
  const context = createContext();
  loadScriptWithExports(context, path.join(__dirname, "../frontend/state/appState.js"), [
    "DEFAULT_STATE",
    "createRepository",
  ...
  assert.equal(manager.getState().ui.isLoading, true);
  services.setMonteCarloResult({ percentiles: {} });
  assert.equal(manager.getState().ui.isLoading, false);
});

test("setChartView stores dedicated chart view", () => {
  const context = createContext();
  loadScriptWithExports(context, path.join(__dirname, "../frontend/state/appState.js"), [
    "DEFAULT_STATE",
    "createRepository",
  ...
  services.setChartView("leverage");

  assert.equal(manager.getState().charts.activeView, "leverage");
});
